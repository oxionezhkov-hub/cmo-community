...
const NETWORKING_EVENT = {
  ...
  intro: '🤝 *Нетворкинг «Разборы CMO»*\n\n2 сентября в 16:00 (МСК) — модерируемый нетворк: у каждого 40 секунд на самопрезентацию, дальше — штурм по запросам, если есть. Длительность 1–1,5 часа.\n\nЧтобы попасть — ответь на несколько коротких вопросов для знакомства (займёт пару минут), и сразу после получишь ссылку на Zoom.',
  // TODO: заменить на реальные вопросы, когда пришлют отдельным сообщением
  questions: [
    'Как тебя представить участникам? (имя, компания/роль)',
    'Чем занимаешься сейчас и в какой нише?',
    'С каким запросом или темой идёшь на нетворк? Что хочешь обсудить или получить?',
    'Чем можешь быть полезен другим участникам? (экспертиза, контакты, опыт)'
  ]
};
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/leadmagnet3") return serveLeadMagnet3();
...
if (url.pathname === '/api/admin/coffee/send-now') {
  ...
  try {
    ...
    await networkingSendReminders(env);
  } catch(err) {
...
async function handleMessage(msg, env) {
  ...
  if (text.startsWith("/start")) {
    ...
    if (payload === NETWORKING_EVENT.startParam) {
      await handleNetworkingStart(msg, env);
      return;
    }
  }

  // Идёт анкетирование на нетворк — следующее сообщение считаем ответом на вопрос
  const networkingState = await env.KV.get(`networking:state:${userId}`, "json");
  if (networkingState && networkingState.eventId === NETWORKING_EVENT.id && text.trim()) {
    await handleNetworkingAnswer(msg, env, networkingState);
    return;
  }
...
// ─── NETWORKING: регистрация по спец-ссылке /start networking0902 ─────
async function handleNetworkingStart(msg, env) {
  const chatId = msg.chat.id;
  const userId = msg.from.id;
  const name = msg.from.first_name || 'участник';
  ...
  let reg = await env.KV.get(regKey, 'json');
  ...
  if (!reg) {
    ...
    const registrants = await env.KV.get(`networking:registrants:${eventId}`, 'json') || [];
...
async function handleNetworkingAnswer(msg, env, state) {
  ...
  const questions = NETWORKING_EVENT.questions;
  ...
  reg.answers = reg.answers || [];
  reg.answers[step] = { question: questions[step], answer: text };
  await env.KV.put(regKey, JSON.stringify(reg));
...
async function networkingSendReminders(env) {
  ...
  const windows = [
    { key: '3h', offsetMs: 3 * 60 * 60 * 1000, label: 'Через 3 часа начинается нетворк' },
    { key: '15m', offsetMs: 15 * 60 * 1000, label: 'Через 15 минут начинается нетворк' },
    { key: '0m', offsetMs: 0, label: 'Нетворк начинается прямо сейчас' }
  ];

  for (const w of windows) {
    const targetTime = startAt - w.offsetMs;
    // Окно допуска — от targetTime до +20 минут, чтобы не пропустить при редком cron-тике
    if (now < targetTime || now > targetTime + 20 * 60 * 1000) continue;

    const sentFlagKey = `networking:sent:${eventId}:${w.key}`;
    const alreadySent = await env.KV.get(sentFlagKey);
    if (alreadySent) continue;
    await env.KV.put(sentFlagKey, '1', { expirationTtl: 60 * 60 * 24 * 7 });

    const registrants = await env.KV.get(`networking:registrants:${eventId}`, 'json') || [];
    let sent = 0;
    for (const tgId of registrants) {
      try {
        await tgSend(env, tgId, `⏰ *${w.label}*\n\n🔗 Ссылка на Zoom:\n${NETWORKING_EVENT.zoomUrl}`);
        sent++;
      } catch (e) {}
    }
    await notifyAdmin(env, `📤 Нетворк 02.09 — разослана ссылка на Zoom (${w.key}) ${registrants.length ? sent + '/' + registrants.length : '0'} участникам`);
  }
}
...
// ─── LEAD MAGNET 3: SPORTPSY174 — СТАРТОВЫЙ МАНДРАЖ (сегмент 1) ──
function serveLeadMagnet3() {
  return new Response(getLeadMagnet3HTML(), { headers: { "Content-Type": "text/html; charset=utf-8" } });
}

function getLeadMagnet3HTML() {
  return `<!DOCTYPE html>
<html lang="ru">
<head>
<meta charset="UTF-8">
...
<script type="text/javascript">
   ...
   k=e.createElement(t),a=e.getElementsByTagName(t)[0],k.async=1,k.src=r,a.parentNode.insertBefore(k,a)})
   (window, document, "script", "https://mc.yandex.ru/metrika/tag.js", "ym");
   ym(12345678, "init", {ssr:true, webvisor:true, clickmap:true, accurateTrackBounce:true, trackLinks:true});
</script>
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* ---------- HERO ---------- */
  .hero { position: relative; border-bottom: 1px solid var(--hair); min-height: 60vh; display: flex; align-items: flex-end; overflow: hidden; }
  .hero-photo {
  ...
  .hero-copy {
    position: relative; z-index: 1; width: 100%; padding: 70px 0 34px;
  }
  .kicker {
  ...
  .inline-photo img { border-radius: 2px; width: 100%; aspect-ratio: 3/2; object-fit: cover; object-position: 78% 35%; }
...
(function () {
  ...
  (function () {
    ...
    var boxes = list.querySelectorAll('input[type="checkbox"]');
    var doneCount = list.querySelector('.done-count');
    var resultBox = document.getElementById('selfcheck-result');
    ...
    function update(fireGoal) {
      ...
      var text;
      if (n >= 4) text = 'Похоже, это тот самый сценарий: технически ребёнок готов, но старт «съедает» результат. Дальше в тексте — почему так происходит и что с этим делать.';
      else if (n >= 2) text = 'Некоторые признаки встречаются — стоит присмотреться внимательнее. Читайте дальше: разбор механизма поможет понять, насколько это про вашу ситуацию.';
      else text = 'Похоже, у вас другая история, но разбор механизма и метода ниже всё равно может быть полезен.';

      resultBox.textContent = text;
      resultBox.classList.add('show');
      if (fireGoal) reachGoal('selfcheck_completed');
    }

    var state = saved();
    boxes.forEach(function (b, i) { if (state[i]) b.checked = true; });
    if (state.length) update(false);
    boxes.forEach(function (b) { b.addEventListener('change', function () { update(true); }); });
  })();

  // Траектория (табы)
  (function () {
    var tabs = document.querySelectorAll('.traj-tabs button');
    var panels = document.querySelectorAll('.traj-panel');
    tabs.forEach(function (tab) {
      tab.addEventListener('click', function () {
        ...
        document.querySelector('.traj-panel[data-panel="' + target + '"]').classList.add('active');
        reachGoal('trajectory_' + target);
      });
    });
  })();

  // Кейс — раскрытие деталей
  (function () {
    var btn = document.querySelector('.case-toggle');
    if (!btn) return;
    btn.addEventListener('click', function () {
      var el = document.getElementById(btn.getAttribute('data-target'));
      el.classList.toggle('open');
      btn.textContent = el.classList.contains('open') ? 'Скрыть детали ↑' : 'Показать детали →';
      reachGoal('case_expanded');
    });
  })();

  // Возражение "дорого" — развилка
  (function () {
    var wrap = document.getElementById('objection-branch');
    if (!wrap) return;
    var buttons = wrap.querySelectorAll('button');
    var answers = document.querySelectorAll('.objection-answer');
    buttons.forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        answers.forEach(function (a) {
          a.classList.toggle('show', a.getAttribute('data-answer-for') === key);
        });
        reachGoal('objection_' + key);
      });
    });
  })();

  // Финальный CTA — копирование текста + переход в Telegram
  (function () {
    var link = document.getElementById('cta-link');
    if (!link) return;
    link.addEventListener('click', function () {
      var text = document.getElementById('cta-text').textContent;
      var status = document.getElementById('copy-status');
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text).then(function () {
          status.textContent = 'Текст скопирован — вставьте его в Telegram';
        }).catch(function () {
          status.textContent = '';
        });
      }
...
async function apiCommunity(request, env, url) {
  ...
  if (action === 'delete-entry') {
    const p = await getPerson(body.id);
    if (!p) return jsonResp({ ok: false, error: 'Не найдено' }, 404);
    const listName = { rating: 'ratings', request: 'requests', note: 'notes', payment: 'payments', status: 'statusChanges' }[body.kind];
    if (!listName) return jsonResp({ ok: false, error: 'Некорректный тип' });
  ...
  if (action === 'edit-entry') {
    ...
    else if (body.kind === 'status') entry.note = body.text || '';
    else entry.text = body.text || '';
    await savePerson(p);
    return jsonResp({ ok: true, person: p });
  }

  if (action === 'set-access-status') {
    const p = await getPerson(body.id);
    if (!p) return jsonResp({ ok: false, error: 'Не найдено' }, 404);
    if (!p.statusChanges) p.statusChanges = [];
    const status = body.status || '';
    if (status) {
      p.accessStatus = status;
      p.accessNote = body.note || '';
      p.statusChanges.push({
        entryId: cmId(),
...
<link rel="icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E%3Crect width='100' height='100' rx='24' fill='%23ff5a36'/%3E%3Ctext x='50' y='68' font-size='58' text-anchor='middle'%3E%F0%9F%92%AC%3C/text%3E%3C/svg%3E">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
...
<link rel="icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E%3Crect width='100' height='100' rx='24' fill='%233457C9'/%3E%3Ctext x='50' y='68' font-size='58' text-anchor='middle'%3E%F0%9F%91%A5%3C/text%3E%3C/svg%3E">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Manrope:wght@400;500;600;700;800&family=Unbounded:wght@600;700&display=swap" rel="stylesheet">
<style>
...
.statusBtn{
  padding:8px 14px; border-radius:10px; border:1.5px solid var(--line); background:var(--bg-soft);
  font-size:12.5px; font-weight:700; cursor:pointer; transition:.15s; color:var(--ink-soft);
}
.statusBtn:hover{border-color:var(--ink);}
.statusBtn.paused.active{background:var(--amber); border-color:var(--amber); color:#fff;}
.statusBtn.problem.active{background:var(--red); border-color:var(--red); color:#fff;}
.statusPopover{width:260px;}
.statusPopover textarea{margin-bottom:0;}
...
.histIcon.status-paused{background:var(--amber-soft); color:#a06b0a;}
.histIcon.status-problem{background:var(--red-soft); color:var(--red);}
...
.histActions{display:flex; gap:10px; opacity:0; transition:.15s;}
.histItem:hover .histActions{opacity:1;}
    ...
    <div class="panelLeft">
      ...
      <div class="section">
        ...
        <div class="statusBtns" style="margin-top:12px;">
          <div class="popoverWrap">
            <button type="button" class="statusBtn paused" id="statusBtnPaused" onclick="clickStatusBtn('paused')">Приостановлен</button>
            <div class="popover statusPopover" id="statusPopover_paused">
              <textarea id="statusReason_paused" class="textInput" rows="3" placeholder="Причина приостановки..."></textarea>
              <div style="margin-top:10px; display:flex; justify-content:flex-end;">
                <button class="btn accent small" onclick="submitAccessStatus('paused')">Сохранить</button>
              </div>
            </div>
          </div>
          <div class="popoverWrap">
            <button type="button" class="statusBtn problem" id="statusBtnProblem" onclick="clickStatusBtn('problem')">Проблема</button>
            <div class="popover statusPopover" id="statusPopover_problem">
              <textarea id="statusReason_problem" class="textInput" rows="3" placeholder="В чём проблема..."></textarea>
              <div style="margin-top:10px; display:flex; justify-content:flex-end;">
                <button class="btn accent small" onclick="submitAccessStatus('problem')">Сохранить</button>
              </div>
            </div>
          </div>
        </div>
...
async function clickStatusBtn(kind){
  if(CURRENT.accessStatus === kind){
    document.querySelectorAll('.statusPopover').forEach(p=>p.classList.remove('show'));
    await callSetAccessStatus('', '');
    return;
  }
  const pop = document.getElementById('statusPopover_'+kind);
  const willShow = !pop.classList.contains('show');
  document.querySelectorAll('.statusPopover').forEach(p=>p.classList.remove('show'));
  if(willShow){
    document.getElementById('statusReason_'+kind).value = '';
    pop.classList.add('show');
    document.getElementById('statusReason_'+kind).focus();
  }
}

async function submitAccessStatus(kind){
  const note = document.getElementById('statusReason_'+kind).value.trim();
  if(!note){ showToast('Укажи причину'); return; }
  document.getElementById('statusPopover_'+kind).classList.remove('show');
  await callSetAccessStatus(kind, note);
}

async function callSetAccessStatus(status, note){
  await ensurePersonCreated();
  const date = todayISO();
  const r = await fetch('/api/community', {method:'POST', headers:authHeaders(), body: JSON.stringify({action:'set-access-status', id:CURRENT.id, status, note, date})});
  const d = await r.json();
  applyPersonUpdate(d.person);
  showToast(status ? 'Статус обновлён' : 'Статус снят');
}
...
function fillPanel(){
  ...
  document.getElementById('statusBtnPaused').classList.toggle('active', p.accessStatus === 'paused');
  document.getElementById('statusBtnProblem').classList.toggle('active', p.accessStatus === 'problem');
  document.querySelectorAll('.statusPopover').forEach(el => el.classList.remove('show'));
...
const HIST_ICONS = {
  ...
  'status-paused': '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="4" width="4" height="16" rx="1"/><rect x="14" y="4" width="4" height="16" rx="1"/></svg>',
  'status-problem': '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M12 9v4M12 17h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/></svg>'
};

function histIconClass(it){ return it.type === 'status' ? ('status-'+it.subtype) : it.type; }
...
function renderHistory(){
  ...
  (p.statusChanges||[]).forEach(e=>items.push({type:'status', subtype:e.status, text:e.note, ...e}));
...
function renderHistViewRow(it){
  return '<div class="histItem">' +
      '<div class="histIcon '+histIconClass(it)+'">'+HIST_ICONS[histIconClass(it)]+'</div>' +
...
<link rel="icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E%3Crect width='100' height='100' rx='24' fill='%231F9D64'/%3E%3Ctext x='50' y='68' font-size='58' text-anchor='middle'%3E%F0%9F%92%B0%3C/text%3E%3C/svg%3E">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Manrope:wght@400;500;600;700;800&family=Unbounded:wght@600;700&display=swap" rel="stylesheet">
<style>
